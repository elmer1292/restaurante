...
// Si la mesa está libre y no tiene comanda activa, mostrar botón para crear comanda
            ...
            <div class="card shadow-sm mb-4">
                ...
                <div class="card-body">
                    <?php if ($mostrarCrearComanda): ?>
                        ...
                        <form method="post" id="formCrearComanda">
                            <input type="hidden" name="id_mesa" value="<?= htmlspecialchars($idMesa) ?>">
                        ...
                        <script>
// Sincronizar variable PHP con JS
const mostrarCrearComanda = <?= json_encode($mostrarCrearComanda) ?>;
...
function enviarProductosComanda() {
    if (mostrarCrearComanda) {
        alert('Primero debe crear una comanda para esta mesa.');
        return;
    }
    if (productosAgregados.length === 0) {
    ...
    const items = productosAgregados.map(p => ({
        id_producto: p.id,
        cantidad: p.cantidad,
        precio_venta: p.precio
    }));
    // Si hay comanda activa, agregar productos a la venta existente
    <?php if ($comanda): ?>
    const idVenta = <?= json_encode($comanda['ID_Venta']) ?>;
    const formData = new FormData();
    formData.append('id_mesa', idMesa);
    formData.append('id_venta', idVenta);
    formData.append('csrf_token', csrfToken);
    formData.append('items', JSON.stringify(items));
    fetch('/restaurante/views/comandas/agregar_productos_comanda.php', {
        method: 'POST',
        body: formData
    })
    ...
    <?php else: ?>
    // Si no hay comanda activa, crear la venta (flujo actual)
    const formData = new FormData();
    formData.append('id_mesa', idMesa);
    formData.append('csrf_token', csrfToken);
    formData.append('items', JSON.stringify(items));
    fetch('/restaurante/views/comandas/procesar_comanda.php', {
        method: 'POST',
        body: formData
    })
    .then(response => response.json())
                                ...
                                <?php $total = 0; foreach ($detalles as $detalle): $total += $detalle['Subtotal']; ?>
                                    <li class="list-group-item d-flex justify-content-between align-items-center">
                                        ...
                                        <div class="d-flex gap-2">
                                            <!-- Botones de estado de producto eliminados -->
...
// Función actualizarEstadoDetalle eliminada
